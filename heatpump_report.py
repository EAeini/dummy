...
Für {inp.houses} Familienhäuser wird eine {inp.heat_pump_type} mit {inp.refrigerant} als Kältemittel überschlägig vorgeplant. Der Report berücksichtigt den angegebenen Jahreswärmebedarf von {num(inp.annual_heat_demand_kwh, 0)} kWh/a, die vorhandene Gebäudehülle mit {num(inp.insulation_cm, 0)} cm Dämmung und {inp.windows}. In der Wirtschaftlichkeitsrechnung ist ein Zuschuss von **{num(inp.subsidy_rate * 100, 0)} % durch {inp.subsidy_source}** auf die Bruttoinvestition angesetzt; die finale Förderfähigkeit muss vor Auftragserteilung verbindlich bestätigt werden.
...
| Zuschussgeber | {inp.subsidy_source} |
...
## 5. Fließbild Kreislauf

```mermaid
flowchart LR
    A[Außenluft] -->|Wärmequelle| B[Verdampfer]
    B --> C[Verdichter]
    C --> D[Verflüssiger / Plattenwärmetauscher]
    D --> E[Expansionsventil]
    E --> B

    D -->|Heizungs-Vorlauf| F[Pufferspeicher / hydraulische Weiche]
    F --> G[Heizkreis Gebäude 1]
    F --> H[Heizkreis Gebäude 2]
    F --> I[Heizkreis Gebäude 3]
    F --> J[Trinkwarmwasser-Speicher / Frischwasserstation]
    G -->|Rücklauf| K[Heizungsrücklauf]
    H -->|Rücklauf| K
    I -->|Rücklauf| K
    J -->|Rücklauf| K
    K --> D
...
**Lesart:** Der obere Ring zeigt den R290-Kältemittelkreis mit Verdampfer, Verdichter, Verflüssiger und Expansionsventil. Der untere Ring zeigt den wassergeführten Heizkreis vom Wärmetauscher über Pufferspeicher/hydraulische Weiche zu den drei Gebäuden und zur Trinkwarmwasserbereitung.
